...
public class RMF_RadialMenu : MonoBehaviour {
    ...
    void OnEnable()
{
    // Initialize your PlayerControls if not already
    if (controls == null)
    {
        controls = new PlayerControls();

        // Read from your "SpellWheel" action
        controls.Gameplay.SpellWheel.performed += ctx => stickInput = ctx.ReadValue<Vector2>();
        controls.Gameplay.SpellWheel.canceled += ctx => stickInput = Vector2.zero;
    }
...
void Update()
    ...
    if (controls != null)
        stickInput = controls.Gameplay.SpellWheel.ReadValue<Vector2>();

    bool joystickMoved = stickInput.sqrMagnitude > 0.01f;
    ...
    if (!useGamepad)
        ...
        if (Mouse.current != null)
        {
            ...
            // Convert to local point relative to the radial menu
            Vector2 localMousePos;
            Camera cam = null;

            // Use canvas camera if needed
            Canvas canvas = rt.GetComponentInParent<Canvas>();
            if (canvas != null && canvas.renderMode != RenderMode.ScreenSpaceOverlay)
                cam = canvas.worldCamera;

            bool success = RectTransformUtility.ScreenPointToLocalPointInRectangle(rt, mousePos, cam, out localMousePos);

            if (success)
                rawAngle = Mathf.Atan2(localMousePos.y, localMousePos.x) * Mathf.Rad2Deg;
